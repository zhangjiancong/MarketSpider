...
# CSV相关
csvfile = open(f'{keyword}-taobao-{time.strftime("%Y%m%d%H%M", time.localtime())}.csv', 'a', encoding='utf-8-sig',
               newline='')
...
# 搜索词与页数获取
gui_text['text'] = '正在操作'
browser.find_element(By.ID, 'q').send_keys(keyword)
browser.find_element(By.CSS_SELECTOR, '#J_TSearchForm > div.search-button > button').click()
browser.implicitly_wait(10)
taobaoPage = browser.find_element(By.CSS_SELECTOR, '#mainsrp-pager > div > div > div > div.total').text

# 爬取页数控制
gui_text['text'] = '等待爬取页数'
print(f'{taobaoPage}，建议每2小时总计爬取不超过20页')
