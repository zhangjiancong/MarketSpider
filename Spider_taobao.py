...
try:
    ...
    browser.inject_cookie("taobao")
    # 发起搜索
    bot_gui.set_text("发起搜索")
    browser.navi_to(f"https://s.taobao.com/search?page=1&q={keyword}&tab=all")
    page_total = browser.find_element_css(
        "#sortBarWrap_right > div > div > div.next-pagination.next-small.next-simple.next-no-border > div > span"
    ).text
    page_total = page_total.split("/").pop()
    ...
    for page in range(page_start, page_end + 1):
        try:
            ...
            try:
                browser.find_element_css(
                    "#search-content-leftWrap > div.leftContent--BdYLMbH8 > div.pgWrap--RTFKoWa6 > div > div > span.next-input.next-medium.next-pagination-jump-input > input"
                ).send_keys(page, Keys.ENTER)
            except Exception as e:
                raise Exception(f"前往页失败:P{page} - {e}")
            ...
            for index, good in enumerate(goods_array):
                ...
                try:
                    ...
                    data["item_name"] = browser.find_element_css(
                        "a> div > div.mainPicAndDesc--Q5PYrWux > div.descWrapper--Ta96FeyX.adaptMod--k5rdyVB_ > div > span",
                        good,
                    ).text
                    data["item_price"] = browser.find_element_css(
                        "a > div > div.mainPicAndDesc--Q5PYrWux > div.priceWrapper--dBtPZ2K1.adaptMod--uU2wpGzc > div > div.innerPriceWrapper--aAJhHXD4",
                        good,
                    ).text
                    ...
                    data["item_shop"] = browser.find_element_css(
                        "a > div > div.shopInfo--Kmh31boz.adaptMod--JjSKtdhD > div.TextAndPic--grkZAtsC > a > div > span.shopNameText--DmtlsDKm",
                        good,
                    ).text
